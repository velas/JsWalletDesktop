...
(function(){
  ...
  changeAmountGeneric = function(field){
    return function(store, amountSend, fast, cb){
      var send, amountBuffer, wallet, token, wallets, feeToken, ref$, feeWallet, decimalsConfig, decimals, balance, maxAmount, homeFee, resultAmountSend, feeType, txType, feeCustomAmount, usdRate, feeUsdRate, account, dataBuilder;
      ...
      homeFee = (function(){
        ...
        default:
          return times(store.current.send.amountSend, div(store.current.send.homeFeePercent, 100));
        }
      }());
      ...
      return dataBuilder.formContractData(function(err){
        ...
        return calcFeeBeforeSend({
          ...
          send.error = (function(){
            ...
            case !(+send.amountCharged > 0 && store.current.send.feeMode === "fixed" && +send.amountCharged < +store.current.send.homeFeePercent):
              return "Amount " + send.amountCharged + " is less than bridge fee (" + store.current.send.homeFeePercent + ")";
            default:
              return "";
            }
          }());
  ...
  out$.changeAmountSend = changeAmountSend = function(store, amountSend, fast, cb){
    ...
    return dataBuilder.formContractData(function(err){
      ...
      return calcFeeBeforeSend({
        ...
        var calcedFee, gasPrice, gasEstimate, txFee, txFeeIn, ref$, minusFee, homeFee, resultAmountSend;
        if (err != null) {
          send.error = err;
        }
        if (err != null) {
