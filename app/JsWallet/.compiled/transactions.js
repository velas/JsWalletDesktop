...
(function(){
  var ref$, each, map, pairsToObj, filter, find, getTransactions, run, task, getPendingTxs, removeTx, applyTransactions, addTask, calcCertainWallet, same, extend, transformPtx, makeNotPending, checkTransactionTask, checkPtxInBackground, checkPtxsInBackground, rebuildHistory, loadAllTransactions, loadWalletTransactions, out$ = typeof exports != 'undefined' && exports || this, slice$ = [].slice;
  ...
  checkTransactionTask = function(bgStore, web3, network, token, ptx){
    return function(store, cb){
      ...
      return check(ptx[0], function(err, data){
        ...
        if ((data != null ? data.status : void 8) === 'confirmed') {
          calcCertainWallet(store, token, function(err){
            return cb(null);
          });
        }
        return cb('pending');
  ...
  out$.checkPtxInBackground = checkPtxInBackground = function(store, web3, network, token, ptx, cb){
    addTask(ptx[0], checkTransactionTask(store, web3, network, token, ptx));
    return cb(null);
  };
  out$.checkPtxsInBackground = checkPtxsInBackground = function(store, web3, network, token, arg$, cb){
    var ptx, rest;
  ...
  out$.loadWalletTransactions = loadWalletTransactions = function(store, web3, token, cb){
    ...
    if (wallet == null || ((ref$ = wallet.txsStatus) === 'loading' || ref$ === 'loaded')) {
      return;
    }
