...
(function(){
  ...
  stakingAccountsContent = function(store, web3t){
    var style, lang, buttonPrimary3Style, goBack, getBalance, getOptions, useMin, useMax, isSpinned, refresh, perPage, page, _index;
    ...
    return web3t.vlx_native.getAddress(function(err, accountAddress){
      var build, cancel, iconStyle, stakerPoolStyle, stats, notificationBorder, blockStyle, createStakingAccount, totalOwnStakingAccounts, ref$, loadingAccountIndex, perPage, page, paginationDisabled, fetchErrorOccurred, svgIcon, children;
      build = function(store, web3t){
        return function(item){
          var index, account, lamports, address, key, rent, seed, status, validator, activationEpoch, deactivationEpoch, active_stake, lockupUnixTimestamp, inactive_stake, balance, balanceRaw, highlight, activeBalanceIsZero, maxEpoch, isActivating, hasValidator, $status, vlx, wallet, walletValidator, undelegate, choose, removeStakeAcc, withdraw, now, lockedAndCanWithdraw, notLocked, authorityCanWithdraw, authorityCanDelegate, canDelegate, canWithdraw, isDisabledWithdraw, canUndelegate, $button, disabled, icon, highlighted, children;
          index = _index++;
          ...
          maxEpoch = '18446744073709551615';
          authorityCanWithdraw = deepEq$(up(item.withdrawer), up(accountAddress), '===');
          authorityCanDelegate = deepEq$(up(item.staker), up(accountAddress), '===');
          canDelegate = (function(){
            switch (false) {
            case !(!hasValidator && authorityCanDelegate):
              return true;
            case !(item.status === "inactive" && hasValidator && authorityCanDelegate):
              return true;
            default:
              return false;
            }
          ...
          canWithdraw = (function(){
            switch (false) {
            case !(authorityCanWithdraw && (notLocked || lockedAndCanWithdraw) && +deactivationEpoch !== +maxEpoch && +store.staking.currentEpoch >= +deactivationEpoch):
              return true;
            default:
              return false;
            }
          }());
          isDisabledWithdraw = (deactivationEpoch != null && activationEpoch != null) && +deactivationEpoch >= +store.staking.currentEpoch;
          canUndelegate = (function(){
            switch (false) {
            case !(authorityCanDelegate && deepEq$(deactivationEpoch, maxEpoch, '===')):
              return true;
            default:
              return false;
            }
          ...
          $button = (function(){
            switch (false) {
            case !(canWithdraw && hasValidator && item.status === "inactive"):
              return button({
            ...
            case !canWithdraw:
              return button({
                ...
                type: 'secondary',
                icon: 'arrowLeft',
                makeDisabled: isDisabledWithdraw
              });
            case !canUndelegate:
              disabled = item.status === 'deactivating' || !authorityCanDelegate;
              icon = (function(){
                switch (false) {
                case !notLocked:
                  return 'arrowLeft';
            ...
            case !(!canDelegate && !canUndelegate && !canWithdraw):
              return button({
                classes: "action-withdraw",
                store: store,
                text: lang.withdraw,
            ...
            : react.createElement('div', {
              ...
              store.staking.accountsAreLoading === false ? react.createElement('div', {
                ...
                filter(function(it){
                  ...
                  canWithdraw = deepEq$(up(it.withdrawer), up(accountAddress), '===');
                  canDelegate = deepEq$(up(it.staker), up(accountAddress), '===');
                  return canWithdraw || canDelegate;
                })(
