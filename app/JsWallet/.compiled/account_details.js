...
(function(){
  var react, reactDom, navigate, getPrimaryInfo, web3, bignumber, getLang, historyFuncs, ref$, getAllActiveStake, creationAccountSubscribe, icon, map, split, filter, find, foldl, sortBy, unique, head, each, findIndex, div, times, plus, minus, velasNodeTemplate, hdkey, bip39, md5, menuFuncs, btoa, Buffer, copiedInform, copy, round5, ethToVlx, vlxToEth, switchAccount, roundHuman, roundNumber, icons, placeholder, epoch, alert, notify, confirm, prompt2, prompt3, button, addressHolder, alertTxn, amountField, seedmem, burger, getErrorMessage, RewardsStats, moment, loader, cb, asCallback, toKeystore, showValidator, Rewards, stakingContent, accountDetails, stringify, fetchEpochRewards, prevEpochData, queryRewardsLoop, get_first_slot_in_epoch, tryGetExtraSlot, get_confirmed_block_with_encoding, retrieveRewardData;
  react = require('react');
  ...
  ref$ = require('../confirmation.ls'), alert = ref$.alert, notify = ref$.notify, confirm = ref$.confirm, prompt2 = ref$.prompt2, prompt3 = ref$.prompt3;
  ...
  stakingContent = function(store, web3t){
    var goBack, style, lang, down, account, address, activationEpoch, deactivationEpoch, rentExemptReserve, checked, stake, stakeInitial, commission, lastVote, lockup, stakers, isValidator, status, myStake, credits_observed, validator, pubkey, lamports, rent, balanceRaw, buttonPrimary3Style, seedStyle, filterIcon, commingSoon, pairs, iStakeChoosenPool, wallet, velasNodeAppliedTemplate, velasNodeAppliedTemplateLine, ref$, accountLeft, accountRight, changeAccountIndex, updateCurrent, accountLeftProxy, accountRightProxy, changeAccountIndexProxy, lineStyle, activate, activateLine, activateString, activateSsh, activateDo, activeClass, activeLine, activeString, activeSsh, activeDo, getBalance, getOptions, useMin, useMax, yourBalance, isSpinned, cancelPool, refresh, removeStakeAcc, withdraw, delegate, undelegate, splitAccount, iconStyle, stakerPoolStyle, stats, hasValidator, active_stake, inactive_stake, delegated_stake, usdRate, usdBalance, usdRent, usdActive_stake, usdInactive_stake, usdDelegated_stake, $validator, activeBalanceIsZero, maxEpoch, myStakeMaxPart, myStakeMaxPartVLX, $status, inactiveStakeLabel, lockupUnixTimestamp, epoch, isLocked, dateExpires, timeExpires, lockupWarningStyle, children, moreStyle, tipStyle, linkStyle, notificationStyle;
    ...
    account = store.staking.chosenAccount;
    address = account.address, activationEpoch = account.activationEpoch, deactivationEpoch = account.deactivationEpoch, rentExemptReserve = account.rentExemptReserve, checked = account.checked, stake = account.stake, stakeInitial = account.stakeInitial, commission = account.commission, lastVote = account.lastVote, lockup = account.lockup, stakers = account.stakers, isValidator = account.isValidator, status = account.status, myStake = account.myStake, credits_observed = account.credits_observed, validator = account.validator, pubkey = account.pubkey, lamports = account.lamports;
  ...
  accountDetails = function(arg$){
    ...
    return react.createElement('div', {
      className: 'staking staking-1346721642'
    }, children = [
      react.createElement('div', {
        style: borderStyle,
        className: 'title'
