...
(function(){
  var react, getPrimaryInfo, getLang, icon, icons, textField, button, copy, round5edit, roundHuman, amountField, ref$, find, map, minus, div, plus, times, CopyToClipboard, copiedPkInform, isValidAddress, subtitleNotifyMsg, alertModal, notificationModal, confirmationModal, swapConfirmationModal, promptModal, promptModal2, promptStakeAuthorizeModal, promptModal3, data, promptChooseTokenModal, promptPasswordModal, $networkDetailsModal, confirmationControl, state, swapConfirm, confirm, notify, prompt, prompt2, prompt3, promptStakeAuthorize, promptPassword, promptChooseToken, alert, networkDetailsModal, toString$ = {}.toString, out$ = typeof exports != 'undefined' && exports || this;
  react = require('react');
  ...
  promptStakeAuthorizeModal = function(store){
    var chosenAccount, pubKey, pubkey, validator, error, confirm, cancel, onTextChange, style, confirmationStyle, inputStyle, inputHolderStyle, buttonStyle, confirmation, lang, buttonPrimary3Style, maxAmountContainer, buttonsContainerStyle, inputErrorsStyle, btnStyle, children;
    if (toString$.call(store.current.stakeAuthorize).slice(8, -1) !== 'String') {
      return null;
    }
    chosenAccount = store.staking.chosenAccount;
    pubKey = chosenAccount.pubKey, pubkey = chosenAccount.pubkey, validator = chosenAccount.validator;
    error = store.current.promptError;
    confirm = function(){
      var callback, promptAnswer;
      if (store.current.promptAnswer == null || +store.current.promptAnswer === 0 || store.current.promptAnswer === "") {
        return;
      }
      store.current.stakeAuthorize = true;
      callback = state.callback;
      state.callback = null;
      promptAnswer = store.current.promptAnswer;
      store.current.promptAnswer = "";
      store.current.promptError = "";
      if (toString$.call(callback).slice(8, -1) === 'Function') {
        return callback(promptAnswer);
      }
    };
    cancel = function(){
      var callback;
      store.current.promptError = "";
      store.current.stakeAuthorize = false;
      callback = state.callback;
      state.callback = null;
      if (toString$.call(callback).slice(8, -1) === 'Function') {
        callback(null);
    ...
    onTextChange = function(e){
      var input, ref$, vlxNativeWallet, network;
      input = ((ref$ = e.target.value) != null ? ref$ : "").trim();
      store.current.promptAnswer = input;
      store.current.promptError = null;
      if (!input.length) {
        return store.current.promptError = 'Address is required';
      }
      vlxNativeWallet = find(function(it){
        return deepEq$(it.coin.token, 'vlx_native', '===');
      })(
      store.current.account.wallets);
      ...
      return isValidAddress({
        ...
        if (err != null) {
          return store.current.promptError = 'Address is not valid';
        }
      });
    };
    style = getPrimaryInfo(store);
    ...
    btnStyle = (function(){
      ...
      switch (false) {
      case error == null:
        return ref$ = {}, import$(ref$, buttonStyle), ref$.opacity = 0.4, ref$;
      default:
        return buttonStyle;
      }
    }());
    return react.createElement('div', {
      ...
      react.createElement('div', {
        style: style = confirmationStyle,
        className: 'header'
      }, store.current.stakeAuthorize), false ? react.createElement('div', {
        style: style = confirmationStyle,
        className: 'text'
        ...
        value: store.current.promptAnswer + "",
        placeholder: "type authorize address...",
        id: "stake-authorize-recipient"
      })), error != null ? react.createElement('div', {
        style: inputErrorsStyle,
        className: 'input-errors'
      }, ' ' + error) : void 8, react.createElement('div', {
  ...
  out$.confirmationControl = confirmationControl = function(store){
    ...
    return react.createElement('div', {}, children = [promptModal3(store), promptStakeAuthorizeModal(store), confirmationModal(store), promptModal2(store), promptModal(store), promptPasswordModal(store), alertModal(store), notificationModal(store), promptChooseTokenModal(store), $networkDetailsModal(store)]);
  };
  state = {
    callback: null
  };
  ...
  out$.promptStakeAuthorize = promptStakeAuthorize = function(store, text, cb){
    store.current.stakeAuthorize = text;
    return state.callback = cb;
  };
