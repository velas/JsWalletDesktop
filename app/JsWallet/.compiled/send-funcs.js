...
(function(){
  var toJS, ref$, times, minus, div, plus, createTransaction, pushTx, getTransactionInfo, changeAmountCalcFiat, changeAmountSend, changeAmount, calcCryptoFromEur, calcCryptoFromUsd, changeAmountWithoutFee, notifyFormResult, getNameMask, resolveAddress, window, navigate, bignumber, close, round, round5, round5edit, roundNumber, roundHuman, topup, getPrimaryInfo, createPendingTx, checkPtxInBackground, map, find, web3, calcFee, confirm, getLang, applyTransactions, getTxDetails, BN, bs58, assert, moment, velasWeb3, icons, abis, out$ = typeof exports != 'undefined' && exports || this;
  toJS = require('mobx').toJS;
  ref$ = require('./math.ls'), times = ref$.times, minus = ref$.minus, div = ref$.div, plus = ref$.plus;
  ref$ = require('./api.ls'), createTransaction = ref$.createTransaction, pushTx = ref$.pushTx, getTransactionInfo = ref$.getTransactionInfo;
  ...
  checkPtxInBackground = require('./transactions.ls').checkPtxInBackground;
  ...
  module.exports = function(store, web3t){
    var lang, send, wallet, feeType, color, primaryButtonStyle, defaultButtonStyle, amountBuffer, currentNetwork, sendTx, walletIcon, performSendSafe, performSendUnsafe, checkEnough, sendMoney, sendEscrow, up, isSelfSend, usdc_to_usdc_velas_swap, usdc_velas_to_usdc_swap, busd_velas_to_busd_swap, busd_to_busd_velas_swap, checkAllowedAmount, checkTxConfirmation, checkApprove, eth_usdtUsdt_velasSwap, usdt_velasEth_usdtSwap, executeContractData, beforeSendAnyway, sendAnyway, toHex, cancel, recipientChange, getValue, amountChange, performAmountEurChange, performAmountUsdChange, amountEurChange, amountUsdChange, encodeDecode, showData, showLabel, whenEmpty, history, network, invoice, token, name, ref$, wallets, feeWallet, feeToken, ref1$, isData, bridgeFeeToken, chooseAuto, chooseCheap, chooseCustom, chosenCheap, chosenAuto, chosenCustom, fromGweiToWei, chooseAutoGasPrice, chooseCustomGasPrice, chosenCheapGasPrice, chosenAutoGasPrice, chosenCustomGasPrice, sendOptions, pending, calcAmountAndFee, flag, useMax, useMaxTryCatch, useMaxAmount, hasSendError, homeFee, homeFeeUsd, FIXED_FEE, PERCENTAGE_FEE, getFeeMode, getBridgeInfo;
    ...
    sendTx = function(arg$, cb){
      var to, wallet, network, amountSend, amountSendFee, data, coin, txType, gas, swap, token, receiver, ref$, chosenNetwork, referTo, recipient, $gasPrice, txObj;
      to = arg$.to, wallet = arg$.wallet, network = arg$.network, amountSend = arg$.amountSend, amountSendFee = arg$.amountSendFee, data = arg$.data, coin = arg$.coin, txType = arg$.txType, gas = arg$.gas, swap = arg$.swap;
      ...
      recipient = (function(){
        switch (false) {
        case !((referTo != null && referTo !== 'vlx_native') && receiver.startsWith('V')):
      ...
      return createTransaction(txObj, function(err, txData){
        ...
        return confirm(store, parts[0], function(agree){
          ...
          return pushTx((import$({
            ...
            var errorMessage, hideErrorMessage;
            if (err != null) {
              ...
              if (errorMessage.indexOf("Unexpected token < in JSON at position 0")) {
            ...
            return createPendingTx({
              ...
              to: send.to,
              from: wallet.address
            }, function(err, pendingTxInfo){
              var currentTransanctionInfo;
              if (err != null) {
                return cb(err);
              }
              ...
              return checkPtxInBackground(store, web3t, network, token, pendingTxInfo, function(){
                return cb(err, tx);
              });
            });
          });
    ...
    checkEnough = function(cb){
      ...
      try {
        amount = minus(minus(minus(wallet.balance, send.amountSend), (ref1$ = wallet.pendingSent) != null ? ref1$ : 0), fee);
        if (+amount < 0) {
    ...
    sendMoney = function(){
      ...
      return checkEnough(function(err){
        ...
        return performSendSafe(function(err, data){
          ...
          if (err != null && (((ref$ = err.message) != null ? ref$ : err) + "").indexOf("Fee amount must be more than 0") > -1) {
            setTimeout(function(){
              return send.error = "";
            }, 1000);
          }
          ...
          return navigate(store, web3t, 'sent');
        });
      });
    };
    sendEscrow = function(){
    ...
    */
    usdc_to_usdc_velas_swap = function(token, chosenNetwork, cb){
      var web3, ref$, FOREIGN_BRIDGE, FOREIGN_BRIDGE_TOKEN, value, receiver, contract, allowedRaw, allowed, coin, gas, gasPrice, amountSend, amountSendFee, feeType, network, txType, ref1$, ref2$, ref3$, minPerTxRaw, minPerTx, maxPerTxRaw, maxPerTx;
    ...
    */
    usdc_velas_to_usdc_swap = function(token, chosenNetwork, cb){
      var web3, ref$, HOME_BRIDGE, HOME_BRIDGE_TOKEN, value, receiver, ref1$, ref2$, ref3$, contract, network, minPerTxRaw, minPerTx, maxPerTxRaw, maxPerTx, err, data;
    ...
    busd_velas_to_busd_swap = function(token, chosenNetwork, cb){
      var web3, ref$, HOME_BRIDGE, HOME_BRIDGE_TOKEN, value, receiver, ref1$, ref2$, ref3$, contract, network, minPerTxRaw, minPerTx, maxPerTxRaw, maxPerTx, data;
    ...
    busd_to_busd_velas_swap = function(token, chosenNetwork, cb){
      var wallets, chosenNetworkWallet, ref$, FOREIGN_BRIDGE, FOREIGN_BRIDGE_TOKEN, web3, contract, value, receiver, allowedRaw, allowed, minPerTxRaw, minPerTx, maxPerTxRaw, maxPerTx;
    ...
    checkAllowedAmount = function(arg$, cb){
      ...
      return confirm(store, "To execute this swap please approve that bridge contract can withdraw your " + token + " and automate payments for you.", function(agree){
        ...
        return createTransaction(txObj, function(err, txData){
          if (err != null) {
            return cb("[check-allowed-amount / create-transaction] err:" + err);
          }
    ...
    checkTxConfirmation = function(arg$, cb){
      ...
      start = arg$.start, token = arg$.token, network = arg$.network, tx = arg$.tx;
      return function(){
    ...
    checkApprove = function(arg$, cb){
      ...
      return checkApprove.timer = setInterval(checkTxConfirmation({
        start: start,
        token: token,
        network: network,
        tx: tx
    ...
    */
    eth_usdtUsdt_velasSwap = function(token, chosenNetwork, cb){
      var web3, ref$, FOREIGN_BRIDGE, FOREIGN_BRIDGE_TOKEN, value, receiver, ref1$, ref2$, ref3$, contract, allowedRaw, allowed, network, data, contractAddress, minPerTxRaw, minPerTx, maxPerTxRaw, maxPerTx;
    ...
    */
    ...
    executeContractData = function(arg$, cb){
      var store, chosenNetwork, token, ref$, wallet, contractAddress, data, dummy, func;
      store = arg$.store;
      ...
      return func(token, chosenNetwork, function(err, data){
        var wallets, chosenNetworkWallet, HECO_SWAP__HOME_BRIDGE, web3, ref$, ref1$, ref2$, ref3$, contract, receiver, network, minPerTxRaw, minPerTx, maxPerTxRaw, maxPerTx, value, ref4$, FOREIGN_BRIDGE, FOREIGN_BRIDGE_TOKEN, ref5$, ref6$, ref7$, ref8$, BSC_SWAP__HOME_BRIDGE, ERC20BridgeToken, ref9$, ref10$, ref11$, contractAddress, amountToSend, ref12$, ref13$, ref14$, ref15$, HOME_BRIDGE, ref16$, value2, sendingTo, referTo, $recipient, recipient, hex, ethAddress;
        if (err != null) {
          return cb(err);
        }
        /* DONE */
    ...
    amountUsdChange = function(event){
      ...
      if ((value != null ? value : "0").toString() === amountBuffer.usdVal.toString()) {
        return false;
      }
      wallets = store.current.account.wallets;
      token = store.current.send.coin.token;
    ...
    feeToken = (ref$ = feeWallet != null ? (ref1$ = feeWallet.coin) != null ? ref1$.nickname : void 8 : void 8) != null ? ref$ : "";
    ...
    getFeeMode = function(token, feeManagerContract){
      var mode, wallets, wallet, evm_wallet, abi, web3, ref$, ref1$, ref2$, ref3$, addr, contract, feeMode, err;
      mode = 'percent';
      ...
      web3 = new Web3(new Web3.providers.HttpProvider(evm_wallet != null ? (ref$ = evm_wallet.network) != null ? (ref1$ = ref$.api) != null ? ref1$.web3Provider : void 8 : void 8 : void 8));
      web3.eth.providerUrl = evm_wallet != null ? (ref2$ = evm_wallet.network) != null ? (ref3$ = ref2$.api) != null ? ref3$.web3Provider : void 8 : void 8 : void 8;
    ...
    getBridgeInfo = function(cb){
      var chosenNetwork, ref$, ref1$, token, ref2$, wallet, network, abi, web3, ref3$, ref4$, HOME_BRIDGE, HECO_SWAP__HOME_BRIDGE, BSC_SWAP__HOME_BRIDGE, FOREIGN_BRIDGE, addr, contract, homeFeePercent, feeMode, feeManagerContract, err, bridgeHomeFee, dailyLimit, currentDay, totalSpentPerDay, remainingDailyLimit, maxAvailablePerTx, minPerTxRaw, minPerTx, maxPerTxRaw, maxPerTx, wallets, walletTo, ref5$;
      ...
      try {
        ...
        maxAvailablePerTx = div(maxAvailablePerTx, Math.pow(10, wallet.network.decimals));
      ...
      wallets);
      ref4$ = walletTo.network, HOME_BRIDGE = ref4$.HOME_BRIDGE, FOREIGN_BRIDGE = ref4$.FOREIGN_BRIDGE, BSC_SWAP__HOME_BRIDGE = ref4$.BSC_SWAP__HOME_BRIDGE, HECO_SWAP__HOME_BRIDGE = ref4$.HECO_SWAP__HOME_BRIDGE;
      web3 = new Web3(new Web3.providers.HttpProvider(walletTo != null ? (ref4$ = walletTo.network) != null ? (ref5$ = ref4$.api) != null ? ref5$.web3Provider : void 8 : void 8 : void 8));
