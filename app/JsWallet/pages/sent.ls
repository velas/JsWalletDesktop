...
module.exports = ({ store, web3t })->
    ...
    lang = get-lang store
    react.create-element 'div', { className: 'sent sent1095625602' }, children = 
        react.create-element 'div', { className: 'animation' }, children = 
            if no-recent
                react.create-element 'img', { style: sent-icon, src: "#{icons.sent-check}", className: 'icon-sent' }
            else if has-pending
        ...
        react.create-element 'div', { style: text-style, className: 'text' }, children = 
            react.create-element 'a', { style: link-style, href: "#{store.current.last-tx-url}", target: "_blank" }, ' ' + lang.transaction
            if no-recent
