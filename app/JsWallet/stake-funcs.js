...
(function(){
  ...
  fillPools = function(arg$, arg1$){
    var store, web3t, onProgress, onFinish, item, rest, stakingAddress;
    ...
    if (['choosestaker', 'info'].indexOf(store.current.page) === -1) {
              ...
              return web3t.velas.Staking.stakeAmount(item.address, stakingAddress, function(err, amount){
                ...
                return web3t.velas.Staking.stakeAmount(item.address, item.address, function(err, nodeStake){
                  ...
                  return web3t.velas.Staking.orderedWithdrawAmount(item.address, stakingAddress, function(err, withdrawAmount){
                    var onFinishLocal, onProgressLocal;
                    if (err != null) {
                      return onFinish(err);
                    }
                    item.withdrawAmount = withdrawAmount;
                    item.myStake = amount;
                    if (isValidatorBanned) {
                      item.status = 'banned';
                    }
  ...
  queryPoolsWeb3t = function(store, web3t, onProgress, onFinish){
    return web3t.velas.Staking.getPoolsInactive(function(err, poolsInactive){
      ...
      return web3t.velas.Staking.getPools(function(err, pools){
        ...
        allPools = map(function(it){
          return {
            address: it,
            status: (function(){
              switch (false) {
              case pools.indexOf(it) === -1:
                return 'active';
              default:
                return 'inactive';
              }
